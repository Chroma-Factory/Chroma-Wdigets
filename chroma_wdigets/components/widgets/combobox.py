...
class ComboBoxBase:
    ...
    def addItem(self, text, icon: Union[str, QIcon, FluentIconBase] = None,
                userData=None):
        """ add item
    ...
    def _showComboMenu(self):
        ...
        for i, item in enumerate(self.items):
            menu.addAction(
                QAction(item.icon, item.text,
                        triggered=lambda x=i: self._onItemClicked(x)))
...
class ComboBox(QPushButton, ComboBoxBase):
    ...
    def paintEvent(self, e):
        ...
        rect = QRectF(self.width() - 22,
                      self.height() / 2 - 5 + self.arrowAni.y, 10, 10)
        if isDarkTheme():
...
class EditableComboBox(LineEdit, ComboBoxBase):
    ...
    def __init__(self, parent=None):
        ...
        self.h_layout.addWidget(self.dropButton, 0, Qt.AlignRight)
